...
func check_suit_against_hand(trick):
	for card in hand_cards:
		if card.suit == trick.suit:
			chip.chip_spot.game.send_message("Trick card MATCHES a suit held in hand")
			return
	chip.chip_spot.game.send_message("Trick card does not match a suit held in hand")
